      ...
      <aside className="w-64 border-r border-slate-800/80 bg-slate-950/85 backdrop-blur-md flex flex-col justify-between sticky top-0 h-screen z-20 shrink-0">
        <div>
          ...
          <div className="h-16 flex items-center gap-2 px-6 border-b border-slate-900">
            ...
            <span className="font-outfit font-bold tracking-wider text-sm">AEGIS ADMIN</span>
            <span className="font-mono text-[9px] font-extrabold px-1.5 py-0.5 rounded bg-rose-950/60 border border-rose-800/50 text-rose-500">ROOT</span>
            {isSqlite && (
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              <div className="overflow-hidden">
                ...
                <span className="font-mono text-[9px] font-bold text-rose-400 uppercase tracking-widest bg-rose-950/30 px-1 rounded border border-rose-900/40">{session.user.role}</span>
              </div>
            </div>
            <ThemeToggle />
          </div>
        ...
        {isSqlite && (
          <div className="bg-amber-950/40 border-b border-amber-800/30 px-6 py-2.5 flex items-center justify-between text-amber-400 text-xs font-mono">
            <div className="flex items-center gap-2">
              ...
              <span><strong>DEVELOPMENT MODE ACTIVE:</strong> The application is running using a local SQLite database fallback because a production MySQL 8+ database is unreachable. All moderation actions and database views are fully persistent locally in SQLite.</span>
            </div>
